...
@ControllerAdvice(basePackages = {"com.buyern.user.controllers"})
public class CustomExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    public final ResponseEntity<Object> handleFileServiceException(FileServiceException ex) {
        logger.error(ex.getMessage());
    ...
    public final ResponseEntity<Object> handleFileUploadException(FileUploadException ex) {
        logger.error(ex.getMessage());
        return new ResponseEntity<>(new ErrorResponse("081", ex.getMessage()), HttpStatus.INTERNAL_SERVER_ERROR);
    }
    @Override
